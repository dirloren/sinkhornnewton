function [pi,errors,time,cgiters] = sinkhorn_newton_primal(mu,nu,cost,gamma,maxiter,check,tol,varargin)
...
for k = 1:maxiter+1
    ...
    [dx,~,~,iters,~] = pcg(H,g,tol,max(round((N1+N2)/24),10),P,[],dx);
    ...
    % assemble plan and compute marginals
    pi = exp(dx(1:N1) / gamma) .* pi .* exp(dx(N1+1:end) / gamma)';
    pie = sum(pi,2);
    pite = sum(pi,1)';
end
toc